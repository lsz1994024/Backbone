...
from Utils.Consts import MIN_TAG_LEN, MAX_TAG_LEN, AA_INFO, AA_RES_MASS, ATOM_MASS
from Parameters import EXTRACT_AA_TOL, AA_SCORE_THRES
...
def readPeaksFromXML(mzXmlPath):
    with mzxml.read(mzXmlPath) as spectra:
        ...
        for spectrum in spectra:
            # print(spectrum)
            # break
            neutralPcMass = spectrum['precursorMz'][0]['precursorMz']*spectrum['precursorMz'][0]['precursorCharge'] - ATOM_MASS['Z']*spectrum['precursorMz'][0]['precursorCharge']
            specDict[int(spectrum['num'])] = [neutralPcMass,
...
def extractTagByCheckAA(mzs, startId, startMz, currentTag, currentScore, aaCanBeHead, extractedTags, lastTag, lastScore, involvedIds, involvedScores, needRevise): #maybe no return
    ...
    candidatesId = getCandidatesForPeak(mzs, startId, startMz)#take followId as new start
    ...
    for candId in candidatesId:
        ...
        realAA, score = findAA(mzs[candId] - startMz, lenCurTags)
        ...
        if realAA != '':#recusion continues
            ...
            if needRevise:
                ...
                candMz = startMz + AA_RES_MASS[realAA]
            ...
            currentTag, currentScore, involvedIds, involvedScores = extractTagByCheckAA(mzs, candId, candMz, currentTag, currentScore, aaCanBeHead, extractedTags, oriCurrentTag, oriCurrentScore, involvedIds, involvedScores, needRevise)
...
def getCandidatesForPeak(mzs, startId, startMz):
    # mz = mzs[startId]
    mz = startMz
    indexes = []
    ...
    for startId in range(len(mzs) - 1):
        ...
        # if 1: #peaks that have been added wont be head later    
            ...
            if startId == 0 or startId == 1:
                needRevise = True
            extractedTags = []
            startMz = mzs[startId]#revise
            extractTagByCheckAA(mzs, startId, startMz, '', 0, aaCanBeHead, extractedTags, '', 0, str(startId), str(0), needRevise)
